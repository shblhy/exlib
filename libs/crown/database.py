...
class Database(object):
    ...
    def create_database(self, safe=False, keep=None, comp=None, replica=None, quorum=None, blocks=None):
        ...
        res = self.execute_sql(
            qc.create_database(self.database, safe=safe, keep=keep, comp=comp, replica=replica, quorum=quorum,
                               blocks=blocks))
        self.get_databases()
        return [[0]] == res
    ...
    def alter_database(self, keep=None, comp=None, replica=None, quorum=None, blocks=None):
        qc = self.get_compiler()
        res = self.execute_sql(
            qc.alter_database(self.database, keep=keep, comp=comp, replica=replica, quorum=quorum, blocks=blocks))
        self.get_databases()
        return [[0]] == res
